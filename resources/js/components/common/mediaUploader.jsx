...
class MediaUploader extends Component {
    ...
    render() {
        const { files, filesToShow, sortColumn, progressLoaded, progressTotal, popperShows } = this.state;
                ...
                <div className="col-12">
                    ...
                    {files.length > 0 && (
                        <Fragment>
                            <div className="row">
                                <div className="col-12">
                                    <Table data={filesToShow} sortColumn={sortColumn} columns={this.state.columns} onDelete={this.handleDelete} popperShows={popperShows} />
                                </div>
                                <div className="col-12 c">
                                    <button className="btn btn-danger m-4" onClick={this.handleUploadFiles} disabled={progressLoaded > 0}>
                                        Upload files
                                    </button>
                                </div>
                                <div className="col-12">
                                    {progressLoaded > 0 && <LinearProgress color="secondary" variant="determinate" value={getProgressCompleted(progressLoaded, progressTotal)} />}
                                </div>
                            </div>
                        </Fragment>
                    )}
